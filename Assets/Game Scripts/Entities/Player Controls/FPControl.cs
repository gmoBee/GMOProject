...
public class FPControl : MonoBehaviour
{
    ...
    [SerializeField] private HeadBobberData headBobberData = null;
    #endregion
    ...
    private HeadBobber m_headBob;
    private PlayerCameraControl m_cameraController;

    private RaycastHit m_hitInfo;
    private IEnumerator m_CrouchRoutine;
    private IEnumerator m_LandRoutine;
    private IEnumerator m_SlideRoutine;
    #endregion
    ...
    */
    ...
    protected virtual void GetComponents()
    {
        ...
        m_cameraController = GetComponentInChildren<PlayerCameraControl>();
        m_yawTransform = m_cameraController.transform;
        m_camTransform = GetComponentInChildren<Camera>().transform;
        m_headBob = new HeadBobber(headBobberData, moveBackwardsSpeedPercent, moveSideSpeedPercent);
    ...
    {
        m_inputVector = playerInputData.MoveDirection.normalized;
    ...
    protected virtual void SmoothSpeed()
    {
        ...
        if (playerInputData.IsRunning && CanRun())
        {
    ...
    protected virtual void CheckIfWall()
    {
        ...
        if (playerInputData.IsMoving && m_finalMoveDir.sqrMagnitude > 0)
    ...
    protected virtual bool CanRun()
    {
        ...
        return _dot >= canRunThreshold && !playerInputData.IsCrouching ? true : false;
    }
    ...
    protected virtual void CalculateSpeed()
    {
        ...
        m_currentSpeed = _moveDir.y == -1 ? m_currentSpeed * moveBackwardsSpeedPercent : m_currentSpeed;
        m_currentSpeed = _moveDir.x != 0 && _moveDir.y == 0 ?
    ...
    {
        if (playerInputData.CrouchClicked && m_isGrounded && m_currentSpeed < runSpeed)
            InvokeCrouchRoutine();
    }

    protected virtual void InvokeCrouchRoutine()
    {
        if (playerInputData.IsCrouching)
            if (CheckIfRoof())
                return;
    ...
    {
        if (playerInputData.SlideClicked && m_isGrounded)
            InvokeSlideRoutine();
    }

    protected virtual void InvokeSlideRoutine()
    {
        if (playerInputData.IsSliding)
            if (CheckIfRoof())
                return;
    ...
    protected virtual IEnumerator SlideRoutine()
    {
        ...
        float _percent = 0f;
        float _smoothPercent = 0f;
        float _speed = 1f / slideTransitionDuration;
        ...
        // Execute Animation
        while (_percent < 1f && playerInputData.IsSliding)
        {
            _percent += Time.deltaTime * _speed;
    ...
    protected virtual void HandleHeadBob()
    {
        ...
        if (playerInputData.IsMoving && m_isGrounded && !m_hitWall)
        {
            if (!m_duringCrouchAnimation) // we want to make our head bob only if we are moving and not during crouch routine
            {
                m_headBob.ScrollHeadBob(playerInputData.IsRunning && CanRun(), playerInputData.IsCrouching, playerInputData.MoveDirection);
    ...
    {
        m_cameraController.HandleSway(m_smoothInputVector, playerInputData.CameraMoveDirection.x);
    }
    ...
    protected virtual void HandleRunFOV()
    {
        if (playerInputData.IsMoving && m_isGrounded && !m_hitWall)
        {
            if (playerInputData.RunClicked && CanRun())
            {
                m_duringRunAnimation = true;
                m_cameraController.ChangeRunFOV(false);
            }

            if (playerInputData.IsRunning && CanRun() && !m_duringRunAnimation)
            {
                m_duringRunAnimation = true;
                m_cameraController.ChangeRunFOV(false);
            }
        ...
        if (playerInputData.RunReleased || !playerInputData.IsMoving || m_hitWall)
        {
    ...
    {
        if (playerInputData.JumpClicked && !playerInputData.IsCrouching)
        {
