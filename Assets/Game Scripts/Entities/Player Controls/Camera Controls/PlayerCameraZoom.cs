...
public class PlayerCameraZoom
{
    ...
    public void Init(Camera _cam, PlayerInputData _data)
    {
        m_inputData = _data;
    ...
    public void ChangeFOV(MonoBehaviour _mono)
    {
        if (m_running)
        {
            m_inputData.IsZooming = false;
            m_zooming = m_inputData.IsZooming;
            return;
        }
    ...
    IEnumerator ChangeFOVRoutine()
    {
        ...
        float _targetFOV = m_inputData.IsZooming ? zoomFOV : m_initFOV;
